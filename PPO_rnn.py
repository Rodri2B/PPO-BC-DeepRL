from utils import Critic, RNNBetaActor, RNNGaussianActor_musigma, RNNGaussianActor_mu
import numpy as np
import copy
import torch
import math
...
class BC_PPO_RNN_agent(PPO_RNN_agent):

	def train(self,obs_ex,action_ex,past_action_ex,trajectory_hist,trajectory_hist_ex):
		self.entropy_coef*=self.entropy_coef_decay
		...
		past_action_expt = torch.from_numpy(past_action_ex).to(self.dvc)
		...
		# Pre-Allocate Memory 
		acumulative_index=0
		for traj_index in range(0,len(trajectory_hist_ex)):
			
			traj_start = trajectory_hist_ex[traj_index]
			traj_end = trajectory_hist_ex[traj_index+1] if (traj_index < (len(trajectory_hist_ex)-1)) else obs_expt.shape[0]
			traj_size = traj_end-traj_start
		...
		acumulative_index*=obs_expt.shape[1]

		obs_past_act_actor_expt = torch.zeros((self.rnn_sequence_length,acumulative_index,self.state_dim+self.action_dim),dtype=torch.float32,device=self.dvc)
		action_actor_expt = torch.zeros((self.rnn_sequence_length,acumulative_index,self.action_dim),dtype=torch.float32,device=self.dvc)
		last_hidden_state_actor_expt = torch.zeros((self.rnn_layers_num,acumulative_index,self.hidden_state_dim),dtype=torch.float32,device=self.dvc)
		...
		for traj_index in range(0,len(trajectory_hist_ex)):
			...
			last_h0 = torch.zeros((self.rnn_layers_num,obs_expt.shape[1],self.hidden_state_dim),dtype=torch.float32,device=self.dvc)

			if(traj_size%self.rnn_sequence_length==0 and traj_size>=self.rnn_sequence_length):
				for sequence_index in range(0,traj_size//self.rnn_sequence_length):
					#np.concatenate([obs,past_action], axis=-1)
					first_index_start = acumulative_index*obs_expt.shape[1]
					first_index_end = (acumulative_index+1)*obs_expt.shape[1]
					second_index_start = traj_start+sequence_index*self.rnn_sequence_length
					second_index_end = traj_start+(sequence_index+1)*self.rnn_sequence_length

					obs_past_act_actor_expt[0:self.rnn_sequence_length,first_index_start:first_index_end,:] = \
					torch.cat([obs_expt[second_index_start:second_index_end],
							   past_action_expt[second_index_start:second_index_end]], dim=-1)

					action_actor_expt[0:self.rnn_sequence_length,first_index_start:first_index_end,:] = action_expt[second_index_start:second_index_end].clone()
					
					with torch.no_grad():
						last_hidden_state_actor_expt[0:self.rnn_layers_num,first_index_start:first_index_end,:] = last_h0.detach() #last_hidden_state[second_index_start].clone()
						gru_out,last_h0 = self.actor.gru(input=obs_past_act_actor_expt[0:self.rnn_sequence_length,first_index_start:first_index_end,:],hx=last_h0)
				...
				pre_first_index_start = (acumulative_index-1)*obs_expt.shape[1]
				pre_first_index_end = (acumulative_index)*obs_expt.shape[1]
				...
				pre_second_index_start = traj_start+((traj_size//self.rnn_sequence_length)-1)*self.rnn_sequence_length
				pre_second_index_end = second_index_start
				...
				with torch.no_grad():
					pre_last_h0 = last_hidden_state_actor_expt[0:self.rnn_layers_num,pre_first_index_start:pre_first_index_end,:]
					pre_sequence = torch.cat([obs_expt[pre_second_index_start:pre_second_index_end],past_action_expt[pre_second_index_start:pre_second_index_end]], dim=-1)
					gru_out,last_h0 = self.actor.gru(input=pre_sequence,hx=pre_last_h0)
		...
		bc_batch_number = obs_past_act_actor_expt.shape[1]//self.a_optim_batch_size
		...
		for i in range(self.K_epochs):
            ...
            #expert
			perm_a = np.arange(obs_past_act_actor_expt.shape[1])
			np.random.shuffle(perm_a)
			perm_a = torch.LongTensor(perm_a).to(self.dvc)
			
			if obs_past_act_actor_expt.shape[1] > obs_past_act_actor.shape[1]: # N batches
				obs_past_act_actor_expt = obs_past_act_actor_expt[:,perm_a[0:obs_past_act_actor.shape[1]],:]
				action_actor_expt = action_actor_expt[:,perm_a[0:obs_past_act_actor.shape[1]],:]
				last_hidden_state_actor_expt = last_hidden_state_actor_expt[:,perm_a[0:obs_past_act_actor.shape[1]],:]
			else:
				obs_past_act_actor_expt = obs_past_act_actor_expt[:,perm_a,:]
				action_actor_expt = action_actor_expt[:,perm_a,:]
				last_hidden_state_actor_expt = last_hidden_state_actor_expt[:,perm_a,:]
			...
			for i in range(a_optim_iter_num):
				...
				index = slice(bc_i * self.a_optim_batch_size, min((bc_i + 1) * self.a_optim_batch_size, obs_past_act_actor_expt.shape[1]))
				# MSE #
				pred_action, last_h_st_expt = self.actor.deterministic_act(obs_past_act_actor_expt[:,index,:],last_hidden_state_actor_expt[:,index,:])

				bc_loss = self.bc_alpha*(action_actor_expt[:,index,:] - pred_action).pow(2).mean()

				# Log Prob #

				#dist_e, last_h_st_expt = self.actor.get_dist(obs_past_act_actor_expt[:,index,:],last_hidden_state_actor_expt[:,index,:])
				#logprob_e = dist_e.log_prob(action_actor_expt[:,index,:]).sum(dim=2, keepdim=True)
