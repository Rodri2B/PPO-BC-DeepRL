...
def append_chunk(exp_traj,filename):
    ...
    with h5py.File(filename, "a") as f:
        if "data" not in f:
            ...
            maxshape = (None, exp_traj.shape[1],exp_traj.shape[2])
            dset = f.create_dataset(
                "data",
                data=exp_traj,
                maxshape=maxshape,
                chunks=True,   # important for resizing
                #compression="gzip", # Optional: saves significant disk space
                dtype='float32'
            )
        else:
            dset = f["data"]
            ...
            # resize and append
            dset.resize((new_size, exp_traj.shape[1],exp_traj.shape[2]))
            dset[old_size:new_size] = exp_traj

def append_traj_length(traj_length,filename):
    # exp_traj shape: (batch_size, dim)
    with h5py.File(filename, "a") as f:
        if "data_sizes" not in f:
            # create dataset with unlimited rows
            maxshape = (None, 2)
            dset_sizes = f.create_dataset(
                "data_sizes",
                data=traj_length,
                maxshape=maxshape,
                chunks=True,   # important for resizing
            ...
            dset_sizes = f["data_sizes"]
            old_size = dset_sizes.shape[0]
            new_size = old_size + 1

            # resize and append
            dset_sizes.resize((new_size, 2))
            dset_sizes[old_size] = traj_length
...
def main():
    ...
    expt_traj_filename = f"expert_traj/{opt.expert_traj}_exp_traj_rnn.h5"
    ...
    while total_expert_traj < opt.expert_traj_number: 
        ...
        first_past_action = 0.5*np.ones(shape=(1,opt.num_envs,opt.action_dim))
        ...
        append_traj_length(np.array([traj_start_point,traj_start_point+total_traj_length-1]),expt_traj_filename)

        print("Trajectory number {} saved! Shape: T:{}, N:{}, D:{}".format(total_expert_traj,total_traj_length,opt.num_envs,exp_traj.shape[2]))

        traj_start_point = traj_start_point+total_traj_length
